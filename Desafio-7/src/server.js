...
const admin = (req, res, next) => {
    if (req.query.admin === process.env.TOKEN) {
        next();
    } else {
...
// POST api/productos
routerProducts.post('/',admin, async (req,res, next) => {
    const {body} = req;
...
routerProducts.put('/:id', admin ,async (req, res, next) => {
    const {id} = req.params;
    const {body} = req;
...
routerProducts.delete('/:id', admin, async (req, res, next) => {
    const {id} = req.params;
